...
export const Navigation = () => {
  ...
  return (
    <nav className="flex items-center justify-between lg:px-40 md:px-20 px-5 py-2">
      <div className="mt-2 absolute left-1/2 -translate-x-1/2 z-50">
        <Stripes width="w-[35px]" height="h-[35px]" small={true} />
      </div>
      <div className="flex items-center">
        {/* <img src="/" alt="alt" className="w-12 h-12" /> */}
        <h1
          className={`${vcrOsdMono.className} tracking-widest font-mono text-white font-medium lg:text-2xl text-xl z-50`}
        >
          <Link
            href="/"
      ...
      >
        {routes.map((link, index) => (
          <Link
            href={link.href}
            key={index}
            className="ml-6 sm:ml-4  text-white hover:z-10"
          >
            {link.text}
          </Link>
        ))}
      ...
      >
        <div
          className={`fixed top-1/4 right-0 items-center justify-center rounded-md p-8 bg-black opacity-75`}
        >
          <div
